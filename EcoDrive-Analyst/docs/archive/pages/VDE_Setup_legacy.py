...
from src.vde_app.components.vde_setup import (
    ...
    render_vde_calculation_context_header,
...
st.set_page_config(page_title="EcoDrive - VDE Calculation Sheet", layout="wide")
ensure_db()
...
def show_if_exists(col, path, *, width=64, caption=None):
    ...
    with col:
        if p and p.exists():
            render_inline_image(p, width=width, caption=caption)
