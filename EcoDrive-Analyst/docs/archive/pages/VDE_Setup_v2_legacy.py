...
from src.vde_app.components.vde_setup import render_vde_setup_workbook_v2


st.set_page_config(page_title="EcoDrive - VDE Calculation Sheet v2", layout="wide")
ensure_db()
...
def main():
    ...
    with st.sidebar:
        st.header("VDE Setup v2")
        st.caption("Parallel workbook-only version. The legacy VDE Setup page remains intact for comparison.")
        st.caption("Display units now live in Scenario Context / Metadata.")
        if st.button("Reset v2 draft", key="reset_vde_setup_v2_draft"):
            st.session_state.pop("vde_setup_workbook_v2", None)
            reset_vde_setup_state(st.session_state, preserve_meta=True)
            st.rerun()

    st.title("VDE Calculation Sheet")
    st.caption("Vehicle demand, roadload, mass, cycle and NET/TOTAL setup.")
