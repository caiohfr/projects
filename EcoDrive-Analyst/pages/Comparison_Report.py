...
def main() -> None:
    with st.sidebar:
        ...
        if qa_col.button("Switch to QA data", key="comparison_report_switch_to_qa", width="stretch"):
            # overwrite=True on every click, not just when the file is
            # missing: a QA db file created by an older version of this
            # code (fewer/different baselines) would otherwise sit there
            # stale forever, and seed_qa_fuelcons_mock_rows() below assumes
            # its exact current baselines exist (a stale file caused a
            # FOREIGN KEY failure here once already). Safe -- this path is
            # always the synthetic, deterministic QA fixture, never real
            # user data, and seed_qa_database() itself refuses overwrite=True
            # on any path outside data/qa/.
            seed_qa_database(DEFAULT_QA_DB_PATH, overwrite=True)
            # seed_qa_database() only populates vde_db (fuelcons_db stays
            # empty by design, since most QA consumers -- tests especially
            # -- need to control their own scenarios). The Comparison
            # Browse filters/presets/counters have nothing to show without
            # actual FuelCons scenarios, so this button also seeds the
            # opt-in mock set covering Fuel Economy present/missing, NET
            # present/missing, and EPA/WLTP examples.
            seed_qa_fuelcons_mock_rows(DEFAULT_QA_DB_PATH)
