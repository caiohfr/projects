...
class EnergyBalanceL0Adapter:
    ...
    def run(self, resolved: ResolvedSystemScenario) -> SystemScenarioResult:
        ...
        if not isinstance(resolved.fuel_estimate_request, FuelEstimateRequest):
            raise TypeError("ResolvedSystemScenario.fuel_estimate_request must be a FuelEstimateRequest when READY.")

        baseline = run_fuel_estimation(resolved.fuel_estimate_request)
        ...
        if resolved.ordered_technology_deltas:
            ...
            delta_result = apply_delta_stack_to_baseline(
                baseline,
                ctx={"energy_value_mj_per_km": _active_demand(resolved.fuel_estimate_request)},
                deltas=normalized,
            )
